    ...
    switch (action) {
      ...
      case "listSubaccounts": {
        ...
        return json({
          ...
          page,
          pageSize,
          // Feeds the 子账号管理 blurb「全局默认（N 票 / N 座）」. The rows exist, so
          // this reads 1 / 1 from the table like every other surface. The fallback
          // used to be 1/1 here, 1/2 in getSettings above and 1/2 on the customer
          // path — three opinions that happened never to surface because a stored
          // value always won. Aligned at 0 so a deleted row cannot make these
          // three pages disagree about who gets free seats.
          defaults: {
