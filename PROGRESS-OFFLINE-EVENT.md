...
| ~~H navbar icon change~~ | ✅ **Shipped 2026-08-05.** Desktop bar is icons + tooltips (labels move to `aria-label` + a 120ms Radix tooltip); the two dropdowns became `Boxes` / `BookOpen` triggers with no tooltip (the panel that opens on hover says more than a chip would). Current page marked by a yellow underbar, and inside the dropdown panels + the mobile menu by a 3px yellow left rule. Mobile stays text — touch has no hover. Ownership is an explicit table at the top of Navbar.tsx, **not** `startsWith(href)`: `/checkout/return` is a top-level route, so deriving it would blank the navbar right after a customer pays. 13 paths verified, one lit each, 404 lights none. |
| 🟠 Every `hover:text-accent-foreground` in the app is a no-op | **Found 2026-08-05 while making the navbar icons respond to hover — needs a repo-wide grep pass, not urgent.** The Brutalist rebrand set `--accent-foreground` to the SAME ink as `--foreground` (index.css: both `0 0% 8%`), so `text-foreground hover:text-accent-foreground` changes nothing. **12 occurrences**, 7 of them in Navbar.tsx (desktop icons now fixed; the mobile rows and text-fallback branches still carry it), plus `pages/DFY.tsx`, `components/solutions/SolutionsSection.tsx`, `components/FeaturedCourses.tsx`, `components/ui/toggle.tsx`, `components/ui/navigation-menu.tsx`. Nobody will ever report this — a dead hover does not look broken, it just makes the site feel lifeless, and the reporter would have to know what was supposed to happen. Fix by picking a real hover target per surface (the navbar went ink → muted `#59595f` at 100ms) rather than by redefining the token, which would repaint everything at once. NOTE `hover:bg-accent` is FINE and must not be swept up in the same pass: `--accent` is yellow (`48 98% 52%`), so those 15 uses do change something. |
